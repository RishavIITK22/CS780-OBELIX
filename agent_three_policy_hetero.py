"""Inference agent for heterogeneous three-policy OBELIX PPO.
...
def _build_from_config(config: Dict[str, object], state_dict: Dict[str, torch.Tensor]):
...
def _load_once() -> None:
    ...
    if missing:
        raise FileNotFoundError(
            f"Missing heterogeneous three-policy checkpoints for: {missing}. "
    ...
    for behavior, path in paths.items():
        state_dict, config = _load_payload(path)
        model, adapter = _build_from_config(config, state_dict)
